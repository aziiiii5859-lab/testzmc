(function () {
  'use strict';
  console.log('[GDA embed.js] loaded — 2026-06-18 UI redesign');

  // ── Configuration ─────────────────────────────────────────────────────────
  var AGENT_URL = 'https://yf-agent.paas.cmbchina.cn';
  // 直连开关（便于验证单进程合并部署）：
  ...
  // ⚠️ 这是 data_agent 自己的业务逻辑，不沉到 shell.js（其他团队的场景不需要
  // 由我们替他们决定是否做域名映射）。shell 传入的 ctx.biBaseUrl 是原始 location.origin，
  // 本文件在 _initUI 里做映射后再赋给 _pageInfo.biBaseUrl。
  ...
  function _resolveBiBaseUrl(origin) {
    // 解析 hostname，仅替换 host 部分（保留 protocol 与 port）
    var a = document.createElement('a');
    a.href = origin || location.origin;
    var host = a.hostname;
    if (BI_DOMAIN_MAP_ENABLED && BI_DOMAIN_MAP[host]) {
      var mapped = (origin || location.origin).replace(host, BI_DOMAIN_MAP[host]);
      console.log('[GDA] BI 域名映射：' + (origin || location.origin) + ' → ' + mapped);
      return mapped;
    }
  ...
  // ── CSS ────────────────────────────────────────────────────────────────────
  // 浮动按钮/遮罩/抽屉外壳由 shell.js 提供（#gdas-btn/#gdas-overlay/#gdas-drawer）；
  // 本文件只负责 shell 分配的 pane 容器内部内容。#gda-tabroot 是这块内容的根节点，
  // 承接原来 #gda-drawer 的内容样式（选择器前缀由 #gda-drawer 改为 #gda-tabroot，规则未变）。
  var _CSS = [
    '#gda-tabroot{display:flex;flex-direction:column;height:100%;background:#FAFBFD;',
    'font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;',
    'font-size:14px;color:#1e2a4a;}',
    '#gda-tabroot *,#gda-tabroot *::before,#gda-tabroot *::after{box-sizing:border-box;margin:0;padding:0;}',

    /* Status row（原头部状态点，随场景一起挂载在自己的 pane 内）*/
    '#gda-status-row{padding:8px 16px 0;flex-shrink:0;}',
    ...
    /* Bubbles */
    '#gda-tabroot .gda-u{align-self:flex-end;max-width:78%;}',
    '#gda-tabroot .gda-u .gda-b{background:linear-gradient(135deg,#2563eb,#1d4ed8);',
    'color:#fff;padding:10px 15px;border-radius:18px 18px 4px 18px;',
    'font-size:13px;line-height:1.55;white-space:pre-wrap;',
    'box-shadow:0 2px 8px rgba(37,99,235,.2);}',
    '#gda-tabroot .gda-a{align-self:flex-start;max-width:92%;}',
    '#gda-tabroot .gda-a .gda-b{background:#fff;padding:11px 15px;',
    ...
    /* Thinking */
    '#gda-tabroot .gda-th{align-self:flex-start;width:100%;border-left:2px solid #bfdbfe;padding-left:12px;}',
    '#gda-tabroot .gda-ch{display:flex;align-items:center;gap:6px;padding:4px 10px;',
    ...
    '#gda-tabroot .gda-ch:hover{background:#e0ebff;}',
    '#gda-tabroot .gda-ch.open{background:#fafbff;}',
    '#gda-tabroot .gda-ch.open:hover{background:#eef3ff;}',
    '#gda-tabroot .gda-arr{margin-left:auto;font-size:9px;opacity:0;transition:opacity .15s,transform .2s;}',
    '#gda-tabroot .gda-ch:hover .gda-arr,#gda-tabroot .gda-ch.open .gda-arr{opacity:1;}',
    '#gda-tabroot .gda-ch.open .gda-arr{transform:rotate(180deg);}',
    '#gda-tabroot .gda-cb{background:#fafbff;border:1px solid #c7d8fa;border-radius:8px;',
    ...
    '#gda-tabroot .gda-cb.hide{display:none;}',

    /* Tool */
    '#gda-tabroot .gda-tb{align-self:flex-start;width:100%;border-left:2px solid #bfdbfe;padding-left:12px;}',
    '#gda-tabroot .gda-th2{display:flex;align-items:center;gap:6px;padding:4px 10px;',
    'background:#f0f5ff;border-radius:20px;border:none;cursor:pointer;',
    'font-size:12px;color:#1e2a4a;user-select:none;transition:background .15s;',
    'width:fit-content;min-width:140px;}',
    '#gda-tabroot .gda-th2:hover{background:#e0ebff;}',
    '#gda-tabroot .gda-th2.open{background:#fafbff;}',
    '#gda-tabroot .gda-th2.open:hover{background:#eef3ff;}',
    '#gda-tabroot .gda-th2:hover .gda-arr,#gda-tabroot .gda-th2.open .gda-arr{opacity:1;}',
    '#gda-tabroot .gda-th2.open .gda-arr{transform:rotate(180deg);}',
    '#gda-tabroot .gda-tn{font-weight:600;}',
    '#gda-tabroot .gda-ts{margin-left:auto;font-size:11px;padding:1px 7px;',
    'border-radius:10px;font-weight:500;flex-shrink:0;}',
    '#gda-tabroot .gda-ts.run{background:#fef3c7;color:#92400e;}',
    '#gda-tabroot .gda-ts.ok{display:none;}',
    '#gda-tabroot .gda-ts.err{background:#fee2e2;color:#991b1b;}',
    '#gda-tabroot .gda-tb2{background:#fafbff;border:1px solid #c7d8fa;',
    'border-radius:8px;margin-top:4px;overflow:hidden;}',
    '#gda-tabroot .gda-tb2.hide{display:none;}',
    '#gda-tabroot .gda-ts2{padding:6px 11px;border-bottom:1px solid #c7d8fa;}',
    '#gda-tabroot .gda-ts2:last-child{border-bottom:none;}',
    '#gda-tabroot .gda-tl{font-size:9px;color:#6b7a99;text-transform:uppercase;',
    'letter-spacing:.06em;margin-bottom:3px;font-weight:600;}',
    '#gda-tabroot .gda-ts2 pre{font-family:Consolas,monospace;font-size:10px;color:#1e2a4a;',
    'white-space:pre-wrap;word-break:break-all;}',

    /* Error / end */
    '#gda-tabroot .gda-er{align-self:flex-start;max-width:92%;}',
    '#gda-tabroot .gda-er .gda-b{background:#fff1f2;border:1px solid #fecdd3;',
    'color:#dc2626;padding:9px 13px;border-radius:10px;font-size:12px;}',
    '#gda-tabroot .gda-se{align-self:center;font-size:11px;color:#6b7a99;',
    'padding:3px 12px;background:#fff;border-radius:20px;border:1px solid #c7d8fa;}',

    /* Typing dots */
    '#gda-tabroot .gda-ty{display:flex;gap:5px;padding:11px 13px;}',
    '#gda-tabroot .gda-ty span{width:6px;height:6px;background:#c7d8fa;border-radius:50%;',
    'animation:gda-bounce 1.2s infinite;}',
    '#gda-tabroot .gda-ty span:nth-child(2){animation-delay:.2s;}',
    '#gda-tabroot .gda-ty span:nth-child(3){animation-delay:.4s;}',
    '@keyframes gda-bounce{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-6px)}}',

    /* Markdown */
    '#gda-tabroot .gda-md-body{white-space:normal;}',
    '#gda-tabroot .gda-md-body p{margin:0 0 7px;line-height:1.7;}',
    '#gda-tabroot .gda-md-body p:last-child{margin-bottom:0;}',
    '#gda-tabroot .gda-md-body strong{font-weight:700;color:#1e2a4a;}',
    '#gda-tabroot .gda-md-body em{font-style:italic;color:#6b7a99;}',
    '#gda-tabroot .gda-md-body code{font-family:Consolas,monospace;font-size:11px;',
    'background:#eff6ff;padding:1px 5px;border-radius:4px;color:#1d4ed8;border:1px solid #dbeafe;}',
    '#gda-tabroot .gda-md-body pre{background:#eef2ff;border:1px solid #c7d8fa;',
    'border-radius:8px;padding:10px 12px;overflow-x:auto;margin:7px 0;}',
    '#gda-tabroot .gda-md-body pre code{background:none;padding:0;border:none;color:#1e2a4a;}',
    '#gda-tabroot .gda-md-body ul,#gda-tabroot .gda-md-body ol{margin:4px 0 7px 18px;}',
    '#gda-tabroot .gda-md-body li{margin-bottom:3px;line-height:1.6;}',
    '#gda-tabroot .gda-md-body h1,#gda-tabroot .gda-md-body h2,#gda-tabroot .gda-md-body h3',
    '{font-weight:700;margin:9px 0 5px;color:#1e2a4a;}',
    '#gda-tabroot .gda-md-body h1{font-size:16px;}',
    '#gda-tabroot .gda-md-body h2{font-size:14px;}',
    '#gda-tabroot .gda-md-body h3{font-size:13px;}',
    '#gda-tabroot .gda-md-body table{border-collapse:collapse;width:100%;margin:8px 0;',
    'font-size:12px;border-radius:6px;overflow:hidden;}',
    '#gda-tabroot .gda-md-body th{background:#2563eb;color:#fff;font-size:10px;',
    'text-transform:uppercase;letter-spacing:.05em;padding:6px 10px;text-align:left;font-weight:600;}',
    '#gda-tabroot .gda-md-body td{padding:5px 10px;border:1px solid #c7d8fa;line-height:1.5;}',
    '#gda-tabroot .gda-md-body tr:nth-child(even) td{background:#eff6ff;}',
    '#gda-tabroot .gda-md-body blockquote{border-left:3px solid #2563eb;margin:7px 0;',
    'padding:4px 11px;color:#6b7a99;background:#eff6ff;border-radius:0 6px 6px 0;}',

    /* Chart */
    '#gda-tabroot .gda-chart-inline-holder{margin:8px 0;}',

    /* Clarify */
    '#gda-tabroot .gda-clarify-card{align-self:flex-start;max-width:92%;}',
    '#gda-tabroot .gda-clarify-card .gda-b{background:#fff;border:1.5px solid #2563eb;',
    'padding:13px 15px;border-radius:4px 18px 18px 18px;',
    'box-shadow:0 2px 8px rgba(37,99,235,.09);}',
    '#gda-tabroot .gda-clarify-q{font-size:13px;color:#1e2a4a;margin-bottom:10px;',
    'line-height:1.55;font-weight:500;}',
    '#gda-tabroot .gda-clarify-opts{display:flex;flex-wrap:wrap;gap:7px;}',
    '#gda-tabroot .gda-clarify-btn{padding:5px 14px;border-radius:20px;',
    ...
    '#gda-tabroot .gda-clarify-btn:hover{background:#eff6ff;}',
    '#gda-tabroot .gda-clarify-btn:disabled{border-color:#c7d8fa;color:#6b7a99;',
    'cursor:not-allowed;background:transparent;}',
    '#gda-tabroot .gda-clarify-btn.selected{background:#2563eb;border-color:#2563eb;color:#fff;}',
    ...
    '#gda-tabroot .gda-wel-title{font-size:18px;font-weight:500;margin-bottom:4px;line-height:1.5;}',
    '#gda-tabroot .gda-wel-title-blue{background:linear-gradient(90deg,rgba(124,46,255,1),rgba(28,96,255,1));',
    '-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}',
    '#gda-tabroot .gda-wel-sub{font-size:12px;color:#505050;margin:14px 0 8px;display:flex;align-items:center;gap:6px;}',
    '#gda-tabroot .gda-wel-caps{margin:0;padding:0;list-style:none;',
    'background:#F7FAFF;border-radius:8px;padding:10px 12px;display:flex;flex-direction:column;gap:10px;}',
    '#gda-tabroot .gda-wel-caps li{font-size:14px;line-height:1.4;color:#4A5565;',
    'display:flex;align-items:center;gap:10px;}',
    '#gda-tabroot .gda-wel-icon{width:23px;height:23px;border-radius:50%;background:#EAEEFF;',
    'display:flex;align-items:center;justify-content:center;flex-shrink:0;}',
    '#gda-tabroot .gda-wel-chips{display:flex;flex-direction:column;gap:8px;}',
    '#gda-tabroot .gda-wel-chip{text-align:left;padding:8px 12px;border-radius:8px;',
    ...
    '#gda-tabroot .gda-wel-chip:first-child{border-color:#065BCE;}',
    '#gda-tabroot .gda-wel-chip:hover{background:#f5f8ff;border-color:#4A81FF;}',
    '#gda-tabroot .gda-wel-chip-arr{opacity:0.3;flex-shrink:0;color:#333;}',
    '#gda-tabroot .gda-wel-loading{font-size:12px;color:#9aa7c2;}',
  ...
  function _injectStyles() {
    if (document.getElementById('gda-styles')) return;
  ...
  // ── DOM creation ───────────────────────────────────────────────────────────
  // container：shell.js 分配给本场景的空 pane 节点（懒挂载，调用一次）
  function _createUI(container) {
    var root = document.createElement('div');
    root.id = 'gda-tabroot';
    root.innerHTML =
      '<div id="gda-status-row">' +
    ...
    // DOM refs
    _chat = document.getElementById('gda-chat');
    _inp  = document.getElementById('gda-inp');
    _bs   = document.getElementById('gda-bs');
    _bx   = document.getElementById('gda-bx');
    _dot  = document.getElementById('gda-dot');
    _st   = document.getElementById('gda-st');

    // Events
    document.getElementById('gda-clear').onclick = _clearChat;
  ...
  function _renderChips() {
    var box = document.getElementById('gda-wel-chips');
    if (!box) return;
  ...
  // 按钮/遮罩/抽屉开合、Esc 关闭均由 shell.js 统一处理（多场景共用一个抽屉）。
  ...
  function _rmTyping() { var e = document.getElementById('_gda_ty'); if (e) e.remove(); }
  ...
  async function _send(overrideText) {
    ...
    var _wel = document.getElementById('gda-welcome');
    ...
    try {
      ...
      if ((_dot.getAttribute('class') || '').includes('run')) _setStatus('', '就绪');
    }
  }
  ...
  if (typeof GD === 'undefined') {
    console.warn('[GDA] GD SDK not found');
    return;
  }

  // container：shell.js 分配的空 pane 节点；ctx：{dsId, biBaseUrl}（shell 统一路由感知，
  // biBaseUrl 为原始 location.origin，域名映射由本文件自己处理）。shell.js 保证 mount 只在
  // "首次点开"或"数据集切换后重新点开"时调用一次，所以这里每次都当作全新会话来初始化。
  async function _initUI(container, ctx) {
    _pageInfo = { dsId: ctx.dsId, biBaseUrl: _resolveBiBaseUrl(ctx.biBaseUrl) };
    _sid = crypto.randomUUID();
    _history = [];
    _suggestions = null;
    _thinkBody = null; _thinkCollapse = null;
  ...
  // ── 注册进多场景外壳（shell.js）；引导桩保证不管谁先加载完都能注册成功 ──────
